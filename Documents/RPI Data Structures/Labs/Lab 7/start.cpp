...
void read_grid(istream& istr, vector<vector<bool> > & blocked_grid, vector<vector<bool> > & trail,
...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y, vector<vector<bool> > const& trail) {
...
int countPaths(int n, int m, int x, int y, vector<vector<bool> > const& blocked_grid, vector<vector<bool> > & trail){
	...
	if (n ==x){
		trail[n][m] = true;
		return countPaths(n, m+1,x, y, blocked_grid, trail);
	}
	if (m == y){
    	trail[n][m] = true;
		return countPaths(n+1, m,x, y, blocked_grid, trail);
	}
    
    // Else count sum of both ways
	trail[n][m] = true;
    return (countPaths(n+1, m, x, y, blocked_grid, trail) + countPaths(n, m+1,x, y, blocked_grid, trail));
}
...
int main(int argc, char* argv[]) {
  ...
  int start_x, start_y;
  read_grid(istr, blocked_grid, trail, start_x, start_y);
  print_grid(blocked_grid, start_x, start_y, trail);
	
  cout<<start_x<<start_y<<endl;

  cout << "Number of Paths " << countPaths(0,0,start_x, start_y, blocked_grid, trail) << endl;
  print_grid(blocked_grid, start_x, start_y, trail);
