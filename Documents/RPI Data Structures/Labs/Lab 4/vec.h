...
template <class T> void Vec<T>::print(){
	for (int i = 0; i<m_size; i++){
		cout<<m_data[i]<<"  ";
		cout<<"M_data = "<<*m_data<<"  "<<"M_size = "<<m_size<<"  "<<"M_alloc = "<<m_alloc<<endl;
	}
	cout<<endl;
}

template <class T> remove_matching_elements(Vec <T>& inputvec, const T& inp){
	Vec <T> temp = inputvec;
	int count = 0;
	inputvec.clear();
	for (int i = 0; i< temp.size(); i++){
		...
		else {inputvec.push_back(temp[i]); }
	}
	return count;
}
#endif
