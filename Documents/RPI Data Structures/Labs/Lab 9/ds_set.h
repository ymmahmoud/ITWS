...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    if(old_root == NULL){ return old_root; }
    
    TreeNode<T>* nnode = new TreeNode<T>(old_root->value);

    nnode->left = copy_tree(old_root->left);
    nnode->right = copy_tree(old_root->right);

    TreeNode<T>* new_root = nnode;
  ...
  iterator find(const T& key_value, TreeNode<T>* p) {
    ...
    while(p != NULL){
      curr = p->value;
      if(curr > key_value){ p = p->left; }
      else if(curr < key_value){ p = p->right; }
  ...
  /*iterator find(const T& key_value, TreeNode<T>* p) {
    std::list<TreeNode<T>*> curr_lvl;
    curr_lvl.push_back(p);
    while(curr_lvl.size() != 0){
      std::list<TreeNode<T>*> nxt_lvl;
      typename std::list<TreeNode<T>*>::iterator itr = curr_lvl.begin();
      for( ; itr != curr_lvl.end(); ++itr){
        if( (*itr)->value == key_value ){ return *itr; }
        else{
